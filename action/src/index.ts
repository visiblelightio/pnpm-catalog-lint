...
async function run(): Promise<void> {
  try {
    ...
    core.info(`Downloading pnpm-catalog-lint ${releaseTag} for ${platformTarget}`);
    ...
    const binaryName = platform === "win32" ? "pnpm-catalog-lint.exe" : "pnpm-catalog-lint";
    ...
    if (exitCode !== 0) {
      throw new Error(`pnpm-catalog-lint execution failed with exit code ${exitCode}`);
    }
  } catch (error) {
